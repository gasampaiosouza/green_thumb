...
function check(elem_class) {
  ...
  if (result_array.length == 3) show_recom(result_array);
}
...
const show_recom = (arr) => {
  ...
  $.ajax({
    ...
    error: (err) =>
      err.readyState == 4
        ? alert(`I didn't find any plant!`)
        : alert(`Something went wrong :(`),
  });
};

const toggleElements = (element_remove, element_show, display) => (
  element_remove.css('display', 'none'), element_show.css('display', display)
);
...
function config_box(box, data) {
  ...
  box
    .querySelector(`.${box.className} > .pick--details > span`)
    .append(water_icon, sun_icon, toxicity_icon);

  return $('#pick--container').append(box);
}

// delete child before append new ones
const delete_child = (elem) =>
  elem.lastChild ? (elem.lastChild.remove(), delete_child(elem)) : 0;
...
function check_icon({ param, data }) {
  let icon_source = {
